...
All you need to know is that early versions of the router had a command injection vulnerability that is now closed. In order to gain ssh access, you first need to downgrade your router's firmware then run the commands listed in [OpenWRT's page](https://openwrt.org/inbox/toh/xiaomi/xiaomi_ax3600).

Downgrading is easy, all you need to do is:
...
I have no idea why I didn't create a bash script to do all that, probably because some of you use Windows :O
...
The router's got a bunch of [mtd] partitions. ssh to your router and see for yourself, they're all there for the taking:
```bash
$ cat /proc/mtd
...
Moreover, in the next post I'll write about the weird stuff I found on the router. Some of these stuff only run if the router's Country Code is set to `CN`, so why not change it to `US`? this is not random, when you change the country code to something other rthan US, the router behaves a bit differently. for instance, it performs online checks against `google` & `www.microsoft.com` instead of `www.baidu.com` & `www.taobao.com`. both are configured in `/etc/config/system` and accessed by running `uci -q get system.netdt.world_domain` and `uci -q get system.netdt.cn_domain` respectively. Don't believe me? take a look at the `check_gateway` function in `usr/sbin/pppoe-check`.
